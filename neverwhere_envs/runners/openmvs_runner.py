...
def run_interface_colmap(colmap_dir, output_mvs, working_dir, image_dir):
...
def run_densify(input_mvs, output_mvs, working_dir, verbose=1):
...
def run_reconstruct(input_mvs, output_mvs, point_cloud, working_dir):
...
def run_refine(input_mvs, input_mesh, output_mvs, working_dir, scales=1, max_face_area=16):
...
def run_texture(input_mvs, input_mesh, output_mvs, working_dir):
...
def main(working_dir, colmap_dir, image_dir):
    """
    ...
    # Run pipeline
    run_interface_colmap(colmap_dir, colmap_mvs, working_dir, image_dir)
    run_densify(colmap_mvs, dense_mvs, working_dir)
    run_reconstruct(dense_mvs, recon_mvs, dense_ply, working_dir)
    run_refine(dense_mvs, recon_ply, refine_mvs, working_dir)
    run_texture(dense_mvs, refine_ply, texture_mvs, working_dir)
