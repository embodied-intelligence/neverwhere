...
from neverwhere_envs.utils.traj import generate_interpolated_path
from torch import Tensor
from torch.utils.tensorboard import SummaryWriter
from torchmetrics.image import PeakSignalNoiseRatio, StructuralSimilarityIndexMeasure
from torchmetrics.image.lpip import LearnedPerceptualImagePatchSimilarity
from neverwhere_envs.utils.misc import (
    AppearanceOptModule,
    CameraOptModule,
...
class Config:
    ...
    # Initial number of GSs. Ignored if using sfm/openmvs
...
def main(data_dir: str, result_dir: str, gpu_index: str = "-1", **kwargs):
    """
    Run 2D Gaussian Splatting training pipeline
    Args:
...
if __name__ == "__main__":
    ...
    ```bash
    # Single GPU training
    python gsplat2d_runner.py default --data_dir /path/to/scene --result_dir /path/to/results

    # Evaluation from checkpoint
    python gsplat2d_runner.py default --data_dir /path/to/scene --result_dir /path/to/results --ckpt /path/to/checkpoint.pt
    ...
    configs = {
        "default": (
            "2D Gaussian splatting training using densification heuristics.",
            Config(
        ...
        "inria": (
            "2D Gaussian splatting training using INRIA's implementation.",
            Config(
    ...
    # Call main directly with the config
    main(**vars(cfg))