...
from neverwhere.schemas import Scene, ImageBackground
...
async def on_camera(event: ClientEvent, sess: neverwhereSession):
    assert event == "CAMERA_MOVE", "the event type should be correct"
    print("camera event", event.etype, event.value)

@app.spawn(start=True)
async def show_heatmap(sess: neverwhereSession):
    sess.set @ Scene()
