...
from neverwhere.cfgs.parkour_go2 import Go2ParkourCfg
...
class Physics(mujoco.Physics):
    """Physics simulation with additional features for the Go2 domain."""
    ...
    TERMINATION_CONTACT_NAMES = ["face_head", "face_dome", "base"]
...
class Go2(base.Task):
    ...
    obs_scales = Go2ParkourCfg.obs_scales
    init_state = Go2ParkourCfg.init_state
    ...
    def before_step(self, action: np.ndarray, physics):
        ...
        clip_value = Go2ParkourCfg.control.clip_actions / Go2ParkourCfg.control.action_scale
        raw_action = np.clip(raw_action, -clip_value, clip_value)
        raw_action = raw_action * Go2ParkourCfg.control.action_scale
